...
from tgedr_dataops.store.local_fs_single_partition_parquet import (
...
def test_10_schema_with_float_and_string():
    tmp_folder = temp_dir()
    dataset_path = os.path.join(tmp_folder, "parquet_float_string")
    schema = pa.schema(
        [
            pa.field("id", pa.int64()),
    ...
    STORE.save(df, key=dataset_path, partition_field="category", schema=schema)
    actual = STORE.get(dataset_path, schema=schema)
    assert 3 == actual.shape[0]
    assert isinstance(actual["price"].dtype, pd.Float64Dtype)
    assert isinstance(actual["name"].dtype, pd.StringDtype)


def test_11_append_and_replace_partitions_error():
    df = pd.DataFrame(
        {
            "id": [1, 2],
            "value": [10, 20],
    ...
    tmp_folder = temp_dir()
    dataset_path = os.path.join(tmp_folder, "error_test")
    
    with pytest.raises(StoreException, match="cannot request for replace_partitions and append"):
        STORE.save(df, key=dataset_path, partition_field="country", append=True, replace_partitions=True)


def test_12_exists():
    # Test _exists method for existing and non-existing paths
    tmp_folder = temp_dir()
    dataset_path = os.path.join(tmp_folder, "test_exists")
    
    # Path doesn't exist yet
    assert not STORE._exists(dataset_path)
    ...
    # Path exists now
    assert STORE._exists(dataset_path)
